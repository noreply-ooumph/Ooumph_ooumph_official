...
def maybe_evolve(client, force: bool = False) -> str:
    ...
    resp = client.messages.create(
        ...
        max_tokens=500,
        system=(
            f"You are a growth strategist for ooumph_official, a Web3/OoumphCoin crypto Instagram.\n"
...
def generate_content(client) -> dict:
    ...
    resp = client.messages.create(
        ...
        max_tokens=1200,
        system=(
            f"You are the content brain for ooumph_official — a Web3, OoumphCoin, and crypto lifestyle Instagram page.\n"
            f"Today is {today}. Niche: {ACCOUNT_NICHE}\n\n"
            "Generate ONE complete Instagram post. Rules:\n"
            "- Topic must feel current and relevant to crypto/Web3 world right now\n"
            "- Caption: 150-200 words. Punchy first line (hook). Build to a strong CTA. No hashtags. No filler.\n"
            "- Tone: visionary crypto builder — bold, direct, forward-thinking. Real, not corporate.\n"
            "- image_prompt: 2 sentences for FLUX AI image gen. Futuristic neon blockchain aesthetic. No text in image.\n"
